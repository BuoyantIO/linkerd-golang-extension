...
import (
	...
	"github.com/linkerd/linkerd-golang-extension/pkg/version"
...
func newCmdInstall() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "install [flags]",
		Args:  cobra.NoArgs,
		Short: fmt.Sprintf("Output Kubernetes resources to install %s extension", extensionName),
		Long:  fmt.Sprintf(`Output Kubernetes resources to install %s extension.`, extensionName),
		Example: `  # Default install.
  linkerd smi install | kubectl apply -f -
  # Install GAMMA extension into a non-default namespace.
...
func render(w io.Writer, valuesOverrides map[string]interface{}) error {
	...
	if err := charts.FilesReader(static.Templates, "linkerd-gamma/", files); err != nil {
		return err
	}
